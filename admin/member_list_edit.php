...
$query = "SELECT * FROM tbl_employee ORDER BY em_id ASC" or die("Error:" . mysqli_error($query));
...
//$i=1;
 while ($row = mysqli_fetch_array($result)) {
 	...
 	echo "<td>" .$row["em_address"] . "</td>";
 	echo "<td>" .$row["em_password"] . "</td>";
 	echo "<td>" .$row["em_email"] . "</td>";
 	echo "<td>" .$row["em_phone"] . "</td>";
 	echo "<td>" .$row["em_designation"] . "</td>";
 	echo "<td>" ."<img src='../mi/".$row["em_images"]."'width='100'>". "</td>";
 	//Edit data
 	echo "<td><a href='member2.php?ID=$row[0]&act=edit2' class='btn btn-success btn-xs'>แก้ไข</a>
</td>";
    echo "</tr>";
 $i++;}
 echo "</table>";
